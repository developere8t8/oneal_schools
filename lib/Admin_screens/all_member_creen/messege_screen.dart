...
import 'package:oneal_school/Admin_screens/Models/memModel.dart';
import 'package:oneal_school/Admin_screens/Models/members.dart';

import 'package:oneal_school/models/chatroom.dart';
...
class _TabMessageState extends State<TabMessage> {
        ...
        child: isLoading
            ...
            : Column(
                children: [
                  ...
                  Padding(
                    padding: const EdgeInsets.only(left: 23.0, right: 23, top: 15, bottom: 5),
                    child: Row(
                  ...
                  Visibility(
                      ...
                      child: Container(
                        ...
                        child: ListView.builder(
                            itemCount: allmembers.isEmpty ? 0 : allmembers.length,
                            shrinkWrap: true,
                            primary: false,
                            itemBuilder: (context, index) {
                              return ListTile(
                                ...
                                trailing: FittedBox(
                                  child: Switch(
                                      value: selectedMem[index].isSelected ?? false,
                                      onChanged: (value) {
                                        setState(() {
                                          selectedMem[index].isSelected = value;
                                          if (selectedMem[index].isSelected == true) {
                                            selected4Message.add(selectedMem[index]);
                                          } else {
                                            selected4Message.remove(selectedMem[index]);
                                          }
                                        });
                                      }),
                                ),
                  ...
                  Padding(
                    padding: const EdgeInsets.only(left: 40.0, right: 40, top: 90),
  ...
  Future getMembers() async {
    ...
    if (snapshot.docs.isNotEmpty) {
      allmembers =
          snapshot.docs.map((e) => MemberData.fromMap(e.data() as Map<String, dynamic>)).toList();

      //owimng memebers

      owingmembers = allmembers.where((element) => element.earmingBalance! > 0).toList();
      //non owing members

      notOwingmembers = allmembers.where((element) => element.earmingBalance! < 0).toList();

      //selected member

      selectedMem = allmembers
          .map((e) =>
              SelectedMem(earningBalance: e.earmingBalance.toString(), id: e.added, name: e.fname))
          .toList();
    }
    isLoading = false;
    setState(() {});
  ...
  Future sendMessages() async {
    ...
    for (var mem in sendMessagesIds) {
      ...
      if (snapchatroom.docs.isNotEmpty) {
        ChatroomModel model =
            ChatroomModel.fromMap(snapchatroom.docs.first.data() as Map<String, dynamic>);
        //arranging messages
        ...
        final newMessage =
            FirebaseFirestore.instance.collection('chatroom').doc(model.id).collection('chats').doc();
        await newMessage.set(messages);
      } else {
        final newChatRoom = FirebaseFirestore.instance.collection('chatroom').doc();
        ChatroomModel newchat = ChatroomModel(id: newChatRoom.id, members: [user.uid, mem]);
