...
class _MemberEditState extends State<MemberEdit> {
  ...
  Future getCategory() async {
    for (var item in widget.data.categories!) {
      QuerySnapshot snapshot =
          await FirebaseFirestore.instance.collection('Categories').where('id', isEqualTo: item).get();
      if (snapshot.docs.isNotEmpty) {
        CategoryModel model = CategoryModel.fromMap(snapshot.docs.first.data() as Map<String, dynamic>);
        setState(() {
          category.add(model);
        });
      }
                          ...
                          children: [
                            InkWell(
                              ...
                              child: const Text(
                                'Update',
                                style: TextStyle(
                                    color: Color(0xff939393), fontSize: 16, fontWeight: FontWeight.w600),
                              ),
                            ),
                            const SizedBox(
                              width: 15,
  ...
  Widget categoryItem(CategoryModel _category, int index) {
    return Container(
        ...
        decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(20.0), border: Border.all(color: Colors.red)),
        padding: const EdgeInsets.only(top: 1.0, bottom: 1.0, left: 30.0, right: 30.0),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Expanded(
            ...
            TextButton(
                onPressed: () async {
                  final delDoc = FirebaseFirestore.instance.collection('Members').doc(widget.data.added);
                  await delDoc.update({
  ...
  Widget categoryItemNew(CategoryModel _category, int index) {
    return Container(
        margin: EdgeInsets.all(5.0),
        decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(20.0), border: Border.all(color: Colors.green)),
        padding: const EdgeInsets.only(top: 1.0, bottom: 1.0, left: 30.0, right: 30.0),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            ...
            TextButton(
                onPressed: () async {
                  ...
                  if (sanp.docs.isNotEmpty) {
                    ...
                    final delDoc =
                        FirebaseFirestore.instance.collection('Members').doc(widget.data.added);
                    await delDoc.update({
