...
import 'package:oneal_school/Admin_screens/Models/admindata.dart';
...
import 'package:oneal_school/Members_screens/Login_member/login_member_page.dart';
import 'package:oneal_school/Members_screens/categories_member_page.dart';
import 'package:oneal_school/Members_screens/deposit_member_page.dart';
import 'package:oneal_school/Widgets/small_btn.dart';
...
//open url
  ...
  Future getTodayTrans() async {
    ...
    if (currentuser != null) {
      QuerySnapshot res = await FirebaseFirestore.instance
          ...
          .where('date', isGreaterThan: Timestamp.fromDate(DateTime.now().subtract(Duration(days: 91))))
          .where('date', isLessThan: Timestamp.fromDate(DateTime.now().add(Duration(days: 1))))
          .where('Parentid', isEqualTo: currentuser.uid)
          .get();

      QuerySnapshot adminsnap = await FirebaseFirestore.instance.collection('Admin').get();
      ...
      setState(() {
        listData = res.docs.map((e) => RecordsData.fromMap(e.data() as Map<String, dynamic>)).toList();
        listData.sort((a, b) => b.date!.compareTo(a.date!));
        if (adminsnap.docs.isNotEmpty) {
          adminData = AdminData.fromMap(adminsnap.docs.first.data() as Map<String, dynamic>);
        }
        if (memSnap.docs.isNotEmpty) {
          memData = MemberData.fromMap(memSnap.docs.first.data() as Map<String, dynamic>);
    ...
    return memData != null
        ? Scaffold(
                      ...
                      Stack(
                        ...
                        children: <Widget>[
                          ...
                          Positioned(
                            ...
                            left: 15,
                            child: Text(
                              memData != null ? '${memData!.earmingBalance} NGN' : '$sum NGN',
                              style: TextStyle(
                                fontSize: 24.sp,
                          ...
                          Positioned(
                              ...
                              child: profileImageUrl.isNotEmpty
                                  ? Container(
                                      child: CircleAvatar(
                                          radius: 52, backgroundImage: NetworkImage(profileImageUrl)),
                                    )
                                  : Container(
                                      child: CircleAvatar(
                                        radius: 52,
                                  ...
                                  if (file == null) {
                                    ...
                                    if (_file != null) {
                                      ...
                                      var snapshot = await firebaseStorage
                                          .ref()
                                          .child('profileImages/${memData!.added}')
                      ...
                      Row(
                        ...
                        children: [
                          const Text(
                            "Filter",
                            style: TextStyle(
                                color: Color(0xff2F2F2F), fontSize: 14, fontWeight: FontWeight.w400),
                          ),
                          SizedBox(
                            width: 6,
                          ),
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        ...
                                        if (listData[index].transtype == 'Credit')
                                          Image.asset(
                                            'assets/icons/Tfee2.png',
                                            scale: 5,
                                          ),
                                        if (listData[index].transtype == 'Debit')
                                          Image.asset(
                                            ...
                                            SizedBox(
                                              ...
                                              child: Row(
                                                children: [
                                                  Expanded(
                                                    child: Text(
                                                      '${listData[index].category} Fee',
                                                      style: TextStyle(
                                                        fontSize: 15,
                                                        fontWeight: FontWeight.w400,
                                                        color: listData[index].transtype == 'Credit'
                                                            ? Color(0xff18D193)
                                                            : Color(0xFF83050C),
                                                      ),
                                                    ),
            ...
            bottomNavigationBar: Padding(
              padding: EdgeInsets.only(left: 17.5, right: 17.5, top: 10),
              child: Row(
  ...
  fetchData() async {
    ...
    if (currentuser != null) {
      QuerySnapshot res = await FirebaseFirestore.instance
          .collection('NewRecord')
          .where('date', isGreaterThan: Timestamp.fromDate(startDate.subtract(Duration(days: 0))))
